...
class huffman {
	std::unordered_map<int8_t, int64_t> value;
	std::vector<node2*> three;
public:
	huffman(){}
	int64_t getValue(int8_t key) {
		return value[key];
	}
	void frequency(int8_t key) {
		bool trovato = 0;
		for (auto& x : value) {
			if (key == x.first)trovato = true;
		}
		if (trovato) value[key]++;
		else value.insert(std::pair<int8_t,int64_t>(key,1));
	}
	void SetNode() {
		std::sort(value.cbegin(), value.cend(), [](std::pair<int8_t, int64_t> c, std::pair<int8_t, int64_t> d) {return c.second < d.second; });
	}
};
void compress(const std::string& filename){
	std::ifstream is(filename,std::ios::binary);
	std::ofstream os("outputto.bin", std::ios::binary);
	bitwriter bw(os);
	huffman prova;
	std::vector<int8_t> caratteri{ std::istreambuf_iterator<char>{is}, std::istreambuf_iterator<char>() };
	for (auto& x : caratteri) {
		prova.frequency(x);
	}
	os.write(reinterpret_cast<const char*>("HUFFMAN1"),8);
	prova.SetNode();
